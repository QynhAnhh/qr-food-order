  ...
  try {
    ...
    if (!isTable) {
      const error = new Error(
        "Chỉ khách hàng quét mã QR mới được dùng chức năng này!",
      );
    ...
    const result = await prisma.$transaction(async (tx) => {
      let currentOrder = await tx.order.findFirst({
        ...
        include: { items: { orderBy: { batchId: "desc" }, take: 1 } },
      });
    ...
    getIo().emit(SOCKET_EVENTS.ORDER_NEW, {
      tableId,
      message: `Bàn ${tableId} vừa gọi món mới!`,
      data: result,
    });

    res
      .status(200)
      .json({ success: true, message: "Đặt món thành công!", data: result });
  } catch (error) {
    next(error);
  }
};
  ...
  try {
    ...
    if (!currentOrder) {
      return res
        ...
        .json({
          success: true,
          message: "Bàn chưa có hóa đơn nào!",
          data: null,
        });
    }
